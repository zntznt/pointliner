...
- **The markdown-first node model: only `para` and `base` are special types.**
  ...
  `#DONE→#TODO`, `> ` for quotes) — never by setting a type flag. **A retyped keyword
  REPLACES the continuation prefix, never stacks (#947, agent-review):** `parseTodo` reads the
