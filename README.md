...
| `{5 to 10}` | an estimate: **7.2 (5–10)** with a distribution sparkline |
...
- **Installable.** Served over https it's a PWA — "Install" gives you a standalone app
...
To start writing: just type. Press `Enter` for a new point, `Tab`/`Shift+Tab` to indent /
...
The outliner is self-explanatory; the generative/computational side has a friendly guide:

- **[The guide](guide/README.md)** — the one big idea, then two families:
  - **[Generating text](guide/generating-text.md)** — alternation, weights, named rules,
    modifiers (`{beast.a}` → "an ogre"), conditionals, decks, oracles, Markov chains.
  ...
  - **[Cookbook](guide/cookbook.md)** — copy-paste recipes: name generator, dungeon
    stocker, yes/no oracle, a self-linting budget, a Fermi estimate, a card deck.
...
DOM), so it stays responsive on large documents. There's no framework and no build —
...
with no cap. Details and a re-run harness in
...
- **`node.text` is plain text, always** — never HTML. Pills are tokens + sidecar records.
