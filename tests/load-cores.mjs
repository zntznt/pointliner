...
export function loadCores() {
  ...
  const need = [
    ...
    'restoreFailMessage',   // #1136: a failed boot restore names the reason
    ...
    'parseRules', 'isYesNoOracle', 'runGrammar', 'expandTemplate', 'resolveBrace', 'condParts', 'condAttempt', 'condAttemptReason', 'condEmptyMessage', 'strCondVerdict',
    ...
    'emptyExtremalProp','depthExtremalProp',   // #1449 a quoted-search min/max that matched nothing (identity, not a division)
    ...
    'collectVars', 'orphanedVarRefCount', 'exprNamesVar', 'freezeOrphanNote', 'shadowedDeclKeys', 'toastGate', 'hintDwell', 'collectRules', 'collectLinks', 'collectBrokenLinks', 'tokenUnderCaret', 'linkCandidates', 'linkCreateOption',
    ...
    'pruneArtifacts','restoreArtifacts','mergeDropped','isChooserExpr','actionPillDesc',   // #1507: what a prune DROPPED, and putting it back; #1571 unions two such reports
    ...
    'braceBodyIsEmptyStub','emptyStubAnnounce',  // #1505: will promotion delete this body, and what it says so
    'varKindLabel',                             // #1353 Phase 3: naming a variable's kind
    ...
    'textBlockPrefix','unfoldTokensIn','artifactTokenRe','normStateKey','boardBlockReason','statusColWarning','baseStructMessage','colWidthLabel','mtColValueCount','mtRowValueCount','blockCmdsPool','slashActiveIdx',   // #1114: state matching, the Board refusal (#1401: and its now-shared unknown-values half), the slash pool
    ...
    'oracleBandOf','oracleBandLive',   // #1499: odds -> band, and the stored band verified against its odds
