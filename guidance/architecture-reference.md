# Pointliner — Architecture Reference

The deep architectural detail that lives behind the concise `CLAUDE.md` entry point.
Read this when your task touches the engine, rendering pipeline, data model, or
persistence layer.

---

## PWA install assets (the sanctioned exception)
...
## For reviewers / other AIs

Read "Core architecture" and "The two-engine reality" below first. The single most
important constraint is that **`node.text` is the source of truth and is plain text** —
every widget is an opaque `[[type:key]]` token in that text plus a record in a sidecar array.
...
discipline: reuse the authoring language, don't grow it.
