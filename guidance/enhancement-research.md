...
- **Was deferred by an existing lock, now partly shipped:** the inline `{a := …}` declaration
  shorthand **shipped** (Stage A, 2026-06-29 — `generation-direction.md` §5 records it), and it
  carries three value kinds: a live formula, a frozen random pick, and (2026-07-27, #952) an
  **uncertain value** whose references correlate. Reference **modifiers** (`a/an`, plural,
  capitalize) shipped too (A1 below). Still deferred: using a pick value in math; per-reference
  re-roll. (`generation-direction.md` §5.) The deferred items appear below as *candidates*;
  moving any of them forward reopens that doc.
...
| **A2** | **Inline lock-and-reuse** `{name = …}` | Perchance `[w=…]`; Tracery saved symbols | — | ⚑ this *is* the deferred `{:=}`-class shorthand; the *declared* random-pick variable already ships the semantics | ✅ **shipped** — `{name := …}` (Stage A) with three kinds: formula, frozen pick, and an uncertain value whose references correlate (#952) |
...
| **B4** | **Deeper units + conformance errors** | Frink, Qalculate | grow `FN1` `from2to` / a general `convert(x,'km','mi')` / `x in mi`; bundle a unit data subset | conversions **P5✓**; dimension-tracking ⚑; **adopt the error-with-fix UX now (P4)** | ◐ partial — the additive slice **shipped** (SR-6/#875): `{= convert(x, from, to)}` over a built-in ratio table + doc-declared `root.units` (File → Custom units), same-dimension only (cross → `#ERR`); `parseUnitDecls`/`convertUnits`/`replaceConvert`. Full dimension-tracking / `x in mi` literals (a new lexical element) still large + out. |
