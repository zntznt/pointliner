...
### UXP-105 ☐ Touch has no visible @ insert door (P2, touch) 🟡  [Batch 2]
...
- **Target:** add an "Insert pill" (@) button to `#edit-bar` under `@media(hover:none)`, routed through the same machinery the @ menu uses — a compact list calling `insertInlineArtifact(id, nodeId, content)` (the path `slashApply` already takes), added alongside the existing eb-btn mousedown+preventDefault so the caret invariant holds. Do NOT synthesize a fake sigil into `checkSlash`. Update the §9 matrix cell with the touch caveat.

### UXP-106 ☐ Emoji `:shortcode:` is the only authoring sigil with no caret-anchored picker (P2/P1) 🟡  [Batch 2]
...
- **Target:** add a `:`-triggered picker on the shared §7.1 pattern sourced from the `EMOJI` map (name+glyph, prefix filter, arrow/Enter/Tab/Esc, `positionCaretMenu`, `role=listbox`/`option` AT wiring). Gate the trigger with a boundary guard mirroring `checkTagTrigger`'s `(?<![a-zA-Z0-9])` so `12:30` doesn't fire. Writes the existing `:shortcode:` form (no new syntax). Add `:emoji:` as a covered GUIDE id, update the matrix emoji P2 cell to credit the new door, and fold the shortcode list into `guide/features.md` in the same change.
