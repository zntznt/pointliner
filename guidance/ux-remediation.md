...
- **Partial (design pass 2):** `:focus-visible` rules added for the search field, zoom title, and all seven artifact pills (solid accent outline; the soft `--ring` glow is now decoration, not the focus indicator). Pills still lack `tabindex`, so the pill rules arm only once pill focus reach lands (explicitly deferred from UXP-15; sequenced with the UXP-19 dedicated pass). Full sweep remains.
