...
- **Pure cores:** `filterBraceForms(prefix)` — pinned in `tests/test.mjs` (structural invariants on every scaffold + `sel` placeholder offsets + engine-recognition of each body + prefix/alias filtering).
