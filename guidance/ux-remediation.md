...
- **Resolved (this PR):** all column operations are consolidated into the existing **Column panel** (the one front door, opened by one clearly-sized ▾ caret button, `aria-label="Column options"`). The panel now carries five sections — **Calculate** (Sum/Average/Count/Min/Max/None), **Alignment** (Left/Center/Right, current shown active), **Insert column** (Left/Right), **Move column** (Left/Right, reusing `mtMoveItem`), and **Delete column** — all keyboard-operable per the §7.1 menu pattern (`role="menu"`/`menuitem`, `↑↓`/`Home`/`End`/`Enter`/`Esc`). The hidden double-click-cycles-alignment handler is **removed**; drag-to-reorder is kept and made discoverable with a **visible grip cue** (grip dots + grab cursor); the column **delete** moved off the header strip into the panel; and the add-column **"+"** is now a full-height, non-clipped right-edge affordance (reveal on hover, always visible on touch). Every control has a real target size and an accessible name; no column op is drag- or double-click-only. *(This subsumes the P2-3 "no UI for column ops" portion; UXP-3 Part B — the arbitrary field-formula cell-reference picker — remains open.)*
...
- **☐ Remaining:** file-menu items, `.collapse-btn` (+`aria-expanded`), `.crumb`, slash-menu `.cmd-item`, `#sc-toggle`/`#storage-warn-close`→`<button>`, `.fn-key`, `.ghost-row`, table handles (`.mt-colh`/`.mt-rowh`/`.mt-delcol`/`.mt-delrow`), `.mt-promote` focus-reach (rides its `#bpop` menu entry, now live).
