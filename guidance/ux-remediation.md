...
### UXP-235 ☐ Children of a bare block flatten up one level in the Markdown export 🟢  [S3-PR5b follow-on]
- **Problem:** `toMarkdown` emits headings, paragraphs and quotes **without a list marker**, so their children have no list item to nest inside. Verified against a real CommonMark implementation: a paragraph with a bullet child renders `<p>Intro</p>` followed by a **top-level** `<ul>`, so the child is a sibling of the paragraph rather than a descendant. The control with a bullet parent nests correctly to full depth, and nesting below the bare block is preserved too, so exactly one level is lost, at the bare-block edge only.
- **Rule:** fidelity of the outline to Markdown mapping (P4-adjacent: the export silently misrepresents the shape of the document).
- **Target:** give bare blocks a continuation-indent model so their subtree stays inside them. This is an exporter redesign rather than a branch fix: every descendant's indent would need to be measured from the nearest enclosing list item's content column instead of from `'  '.repeat(depth)`.
- **Note:** S3-PR5b's blank-line policy already fixed the *adjacent* symptom. Before it, the child list had no blank line before it and was a lazy continuation swallowed into the parent paragraph, which was strictly worse than being a sibling. What remains is nesting depth only.

### UXP-236 ☐ The `innerHeight - 8` popup clamp is hand-copied at six more sites 🟢  [S3-PR6 follow-on]
- **Problem:** S3-PR6 fixed the two positioners that place `#bpop` (`showBulletPopup`, `showSelTypeMenu`) to clamp against `bottomStackHeight()` instead of the raw viewport bottom. The same `window.innerHeight - 8` literal still appears at six other popup sites (the slash menu, the link/mention menus, the table cell popup and friends). They share the defect in principle: any of them can park its last rows under a touch bar, where the bar wins the hit test and a visible control is untappable.
- **Rule:** P3 (reachable — a control that renders but cannot be touched is not reachable).
- **Target:** one shared positioner rather than six more edits. Note that `mtOpenMenu` is the only site with real clamp logic today (added after `#415`), so it is the natural place to grow one from. Not done here because only the bullet menu had a measured failure, and extracting a positioner across six call sites with different anchors, flip strategies and z-index contexts is its own change with its own regression surface.
- **Note:** one further `#bpop` site (the to-do picker) clamps by flipping ABOVE the anchor rather than parking at the bottom, so it does not produce this failure and is not part of the count.
