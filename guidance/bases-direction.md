...
- **Promote (table → base) — ✅ shipped:** "Convert to base" on a static table (a hover-revealed `▦ base` button at the table's top-right; always visible on touch) splits any surrounding content into sibling points — table-in-the-middle becomes point · base · point; table-at-the-end becomes point · base; a point that *is* just the table converts in place. The base then owns its own point. Children stay attached to the node that keeps the original id; the table's `#+TBLFM:` recipe rides along and recomputes in the new base; the whole move is one undo step. Pure core: `planTablePromote(text, l0, l1)` — the block's line range is stamped on the rendered table by `mdToHtml` as `data-l0`/`data-l1`, and the plan returns `null` for anything that isn't a valid table block, so a stale or mirrored range bails instead of corrupting text. The button is suppressed in search results, link mirrors, and the zoomed-title view, where the action can't apply.
