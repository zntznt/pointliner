...
### UXP-246 ✓ Ten more dialogs silently discarded everything you typed 🟡 [dialogs] (RESOLVED 2026-07-25)
- **How it was found — the method is the point.** UXP-244 fixed drafts-survive-dismissal in the pack editor and read like one careless surface. It was not: it was the visible edge of a **rule applied 5 times out of 20**. With every never-reached surface already covered, this pass changed the selection principle from "audit a surface" to "audit a RULE across every surface it governs", starting with `ux-definition-of-done.md` §4 "Drafts survive dismissal".
- **Problem, driven:** every dialog built by the shared `openInsertDialog` destroyed typed input on Escape. `cancel()` went straight to `closeIo()`, which does `ioCard.innerHTML = ''`.

  | dialog | typed | draft kept |
  |---|---|---|
  | `openGrammarDialog` | 90 chars | no |
  | `openMarkovDialog` | 62 | no |
  | `openDeckDialog` | 55 | no |
  | `openSeqDialog` | 29 | no |
  | `openMathDialog` | 26 | no |
  | `openQBaseDialog` / `openQueryDialog` | 22 each | no |
  | `openEstimateDialog` | 12 | no |
  | `openDiceDialog` | 10 | no |
  | `openVarDialog` | 7 | no |

  **0 of 10. 335 characters destroyed in a single pass.** The irony: `_builderFormDrafts`, UXP-180's fix for this exact rule, is cleared a few lines away in `closeBuilderWindow` — the builder's own embedded forms kept drafts while the dialogs they open did not.
- **The instrument had to be fixed first, twice.** The first driver used `eval` and was killed by the app's CSP (`unsafe-eval` blocked — worth knowing for every future driver). The second dispatched a synthetic `KeyboardEvent` on `document` and reported the same 0 of 10 — but the **control** (the pack editor, fixed hours earlier and verified 9/9) also came back empty, which meant the driver was blind rather than the app broken. Re-run with real Playwright keypresses, the control came back **KEPT** while all ten came back empty. Only then was 0-of-10 a fact about the app.
- **Fixed (owner's call: keep per dialog kind, restore only into a matching form).** One draft slot per dialog kind at the shared choke point, plus two pure cores. `insertDraftFrom(values, seeds)` stores the typed values **and the values the dialog was seeded with**; `insertDraftMatches(draft, seeds)` allows restoration only when the dialog opens in the SAME state the draft was abandoned from.
- **The seeds are what make this safe where the pack editor's simpler rule would not have been.** An insert dialog reopens at a different caret for a different insertion, and EDITING a pill seeds the dialog from that pill's own definition. A draft taken over a blank insert can never match the seeds of an existing pill, so it can never shadow a real definition — while a blank insert reopening blank, or the same pill reopened, both match and get their work back. It also handles the deck dialog's default body (`shuffle: a | b | c`), which a "keep it when non-empty" test would have mistaken for typed content.
- **The four hand-rolled dialogs, and what they exposed.** `openVarDialog` turned out **not** to use the shared builder at all — nor do `openPropsDialog`, `openDueDateDialog`, `openAppearanceDialog`. They build their own DOM because `openInsertDialog` cannot express a picker grid, a segmented control, a repeating row list or a date row. All four therefore missed the choke-point fix. They now conform via `wireDialogDraft`, which delegates to the **same two pure cores** rather than growing a fifth mechanism — but the structural cause (opting out of the builder forfeits every later improvement) is filed as **UXP-247**, open.
- **Per-subject keys where the subject matters:** the schedule and property drafts are keyed by the point (`due-dialog:<id>`, `props-dialog:<ids>`), so a half-typed repeat phrase or property set comes back to the same point and never leaks onto the next one opened. The appearance dialog re-renders **in place** after every swatch commit, so it restores only on a fresh open (`_apprFresh`, read before anything sets the flag) — restoring mid-rebuild would push a stale name over the input being typed into.
- **Verified by driving.** 10/10 shared-builder dialogs keep drafts; 10/10 hand-rolled dialogs keep drafts; and **8/8 on the safety property**, which is the risk this change introduces: editing an existing grammar shows ITS definition and not an abandoned draft; the blank insert still gets its own draft back; a half-typed repeat on Point A does not appear on Point B but does return on Point A; property rows likewise; a committed draft is cleared so the next insert opens blank; and **no dismissal wrote anything to the document**.
- **Tests 1606 → 1607** (the fan-out is DOM wiring, so it is source-pinned per dialog; the two decision cores are seeded pins).

### UXP-248 ✓ Nine shared controls sat under the 24px tap floor on touch 🟡 [touch] (RESOLVED 2026-07-25)
- **Method — the rule was CHOSEN by measurement, not by hunch.** Continuing the audit-by-class pass, two rules were triaged across every surface that has one before either was fixed. **P3-1 (accessible name)** came back **0 unnamed and 0 title-only across 25 surfaces** — the `accessibility.md` work genuinely held, and auditing it further would have been motion without value. The **24px tap floor** (`accessibility.md` guardrail 5 / DoD §3) came back with **16 distinct controls under it**, so that is the one that got fixed. Reporting a clean rule as clean is half the point of auditing by class.
- **Measured by HIT-TESTING, not by `getBoundingClientRect`.** The guardrail explicitly permits padding or an invisible `::after` overlay (the docked-strip `inset:-8px` recipe) to extend a target past its visual box, and `.bullet` and `.cap-go` do exactly that — a raw-rect audit calls those conformant controls failures. The driver instead probes `elementFromPoint` outward from each control's centre and reports what the browser will actually resolve a tap to. **Control run first:** `matchMedia('(hover:none)').matches === true` in the test context, because if the media query had not been matching, every number would have described the desktop layout. (Same class of instrument error as the UXP-246 driver, checked for deliberately this time.)
- **Result: 9 shared control classes, ~100+ instances.** `.close-btn` **19x16** (the worst, and design-language §4 makes it the *one* dismiss affordance, so every conforming dialog inherited it) · `.prop-chip` 21 tall (22 instances across 9 surfaces) · `.io-chip` 22 (26 instances, 3 dialogs) · `.guide-nav-btn` 23 (30 instances) · `.ag-toggle` 22 · `.tl-toggle` 23 · `.fm-title` 21 · `.collapse-btn` 23 effective · `.cap-dest-name-btn` 20.
- **Fixed by the overlay path, which for `.ag-toggle` was the only legal option.** An invisible `::after` lifts the hit area and leaves the visual box untouched. That is not merely tidier: `.ag-toggle`'s 22px height is pinned by the canon-chip design test (#397), so growing the box would have traded an accessibility fix for a design regression. Growth is vertical-only wherever a control sits in a row of text or chips.
- **Three controls needed something else, and each cause was proven before it was fixed:**
  - `.collapse-btn` was **24px wide and still measured 23** — nothing clipped it; the **bullet's own `-5px` overlay reached back over its right edge**. The bullet is the app's most important touch target, so the chevron grows LEFT into empty gutter and yields rather than re-fighting the same pixel.
  - `.guide-nav-btn` and `.fm-title` live inside **scrollable panes** (`overflow:auto`), which clip any overlay — for those two the overlay path is unavailable and the visual box has to grow, by padding (with matching negative margin on the title so the head row does not move).
  - `.cap-dest-name-btn` **already declared the `-8px` overlay** and was clipped **twice**: by its parent `.cap-dest-btn` (corner rounding) and by itself (`overflow:hidden` powering the inbox-name ellipsis). The recipe had been applied and then silently defeated at two levels. The parent simply stops clipping; the button still must clip or the name stops truncating, so it uses `overflow:clip` with an 8px `overflow-clip-margin` — text truncates exactly as before, the overlay survives. **The fix pattern already existed one line above** as `.cap-chip{overflow:visible}`, written for this precise trap and never applied to the sibling container.
...
### UXP-251 ✓ User-facing copy still called the document an "outline" 🟢 [vocab] (RESOLVED 2026-07-26)
- **Method:** ux-discipline §1 / V-1 audited **statically over the strings a user actually sees** (1740 of them: command labels and descs, `aria-label`s in HTML and JS, titles, placeholders, `textContent`, toasts, hints, announcements, GUIDE bodies, plus `guide/features.md` and `README.md`). Deliberately NOT a grep of the source: V-2 keeps `node`/`artifact` in the code, so a whole-file grep is almost entirely false positives. The banned-term list was applied with the standard's own carve-outs encoded, because several banned words have legitimate senses.
- **The headline finding is a decision that quietly expired.** The first-run banner button read **"Start a blank outline"** — the banned document-sense — and carried a comment saying so on purpose: `// UXP-185: button label stays (ux-discipline §L81)`. UXP-185 had explicitly kept the banned wording *because the standard prescribed that exact string*, with the rider "if that string changes, update the doc in the same PR." **`ux-discipline.md` no longer contains the string anywhere.** The line the exception pointed at is gone, so the exception had nothing left holding it up — and nothing flagged that, because the comment cites a line number, and line numbers drift (which `CLAUDE.md` warns about in another context entirely).
- **A command that disagreed with itself:** `rollpick` is labelled **"Roll on your document"** while its own `desc` said "your own **outline** is the table", its GUIDE entry was titled "Roll on your own outline", the roll-palette template was labelled "roll on outline", and a nudge cited the old guide title. One feature, four strings, two vocabularies.
- **Fixed (9 sites):** the banner button + its three code comments, the palette template label, the command desc, the GUIDE entry, the nudge that cites it, `guide/features.md` ("Self-checking outlines" → documents; "Build a connected notebook" → "a connected folder of documents"), and `README.md` ("a multi-document workspace" → folder; "a random point from your own outline" → document). `guidance/ux.md` was updated in the same change, honouring UXP-185's own rider.
- **What was deliberately NOT changed, because §1 permits it:** `vlist`'s `aria-label="Outline"` (that IS the navigable tree view), features.md's "the surface is an **outline** … like a foldable to-do list or a notebook" (the view, plus a simile for the reader, not the folder concept), the README screenshot alt text, and README's "a cloud workspace" (describing what Pointliner is *not*). A blanket ban on the word would have been the wrong fix and would have broken the tree view's accessible name.
- **Left open as UXP-252:** "Roll palette". It is the one hit where the ban is arguable (the §1 row describes a *keyboard-navigable* overlay list; this picker is touch-only) and where the fix renames a shipped feature — an owner's call.
- **Tests 1609 → 1610.** The pin asserts both directions: the fixed sites stay fixed, the expired string cannot return, and the legitimate "Outline" tree-view label is protected from a future over-correction.

### UXP-253 ✓ `shuffle:` and `markov:` typos said nothing at all 🟡 [feedback] (RESOLVED 2026-07-26)
- **Method:** P4-1 ("no silent failure: every rejected input signals why") audited by feeding each authoring surface input it MUST reject, then asking what the user is left looking at. Three outcomes were distinguished, because they are different defects: a stated reason (conformant), a bare mark (P4-2 violation), and nothing at all (P4-1 violation).
- **The instrument was wrong twice, and the second correction was the whole finding.** Run 1 shared one page across cases and read a shared live cue element, so a leftover nudge ("Nice, that is a live pill") was scored as a response to `{2dX}`. Run 2 used a fresh page per case and reported **everything** silent — also wrong, because it committed the text and re-rendered. The app signals an unrecognized `{…}` in **EDIT MODE**: `editModeHTML` wraps it in `<span class="gr-src gr-bad" title="Not recognized, so it stays plain text">`, and typing announces the same sentence to the live region. Committing destroys both. **Authoring IS edit mode**, so that is where the rule has to be judged. Run 3 was validated by CONTROLS: `{2d6}` and `{= 2+2}` come back unmarked with a real `.gr-src`, so the probe can tell valid from invalid rather than reporting everything as broken.
- **The defect, once measured properly:** the app applies a **KEYWORD-COMMIT** doctrine — once a body opens with `seq `, `rule `, `oracle:`, `meter:` or `date`, a tail that will not parse is `'invalid'` (marked, with a reason) rather than silently treated as prose. **`shuffle:` and `markov:` were the only two keyword forms never given that treatment.** So `{shuffle: }` and `{markov: }` fell through to `'literal'` and said nothing, while the identical mistake after `seq ` was marked. Same mistake, opposite treatment, decided by which keyword you typed.
- **Fixed** by adding the two commits immediately after their parse attempts, so a body that genuinely parses still wins. 7 of 10 rejected inputs were cued before; **9 of 10 after**.
- **A false rationale was caught before it shipped.** The first version of the code comment claimed the fix also closed a shred hole, citing `{shuffle: a, b | c}` promoting to a coin flip between the literal "shuffle: a, b" and "c" — the failure the `seq` comment describes. Driving it showed that body classifies `artifact`, because `seqParts` accepts comma-separated items. The comment now says so explicitly ("Unlike `seq`, there is no shred to close here… Verified, not assumed"). A plausible-sounding rationale copied from a neighbouring case is exactly the kind of thing that survives review.
- **Regression-checked:** every valid form still classifies `artifact` (`shuffle: a | b | c`, `shuffle 3: a | b | c`, both markov arrow spellings), prose still classifies `literal` — including `{shuffling the deck}`, which starts with the keyword's letters but has no colon.
- **Left open as UXP-254:** `{2dX}`, the last silent case. Dice is the only form whose keyword is its SHAPE, so committing on it changes parsing semantics with a real false-positive surface — an owner's call, not an auditor's.
- **Tests 1610 → 1611.**

### UXP-255 ✓ "Markdown" was lowercase in user-facing copy, and the rule had no guard 🟢 [vocab] (RESOLVED 2026-07-26)
- **Method:** §6 says *'Labels are sentence case; "Markdown" is always capitalized in user-facing copy.'* Its neighbour, the em-dash ban, has a comprehensive drift guard over README + the whole `guide/` tree + GUIDE bodies + command descs. The Markdown half had **no guard at all**, so it was audited the same way and then given the same guard.
- **Found:** 8 sites — a `flashHint` the user sees ("Edit it as plain markdown; undo restores the base."), three in `README.md`, and four more across `guide/` (`README.md`, `computing-numbers.md`, `writing-and-formatting.md`, `tasks-and-organizing.md`, `solo-rpg/session-prep/session-prep.md`).
- **The guard found more than the audit did, which is the point of writing it.** The hand-run scan covered `guide/features.md` + `README.md` and found 4. The guard walks the whole tree and strips code spans properly, and immediately failed on four more — including one my grep had hidden, because `grep -v '\`'` drops a whole LINE containing a backtick anywhere, while the rule only exempts the code span itself.
- **Exemptions match the em-dash guard exactly:** `guidance/` and code comments are internal, and a lowercase `markdown` inside backticks, a fenced block, a link target or a `.md` path is an identifier or filename, not prose.
- **Also audited in the same pass, all clean:** sentence case on 562 labels (every Title Case hit was a sentence start or a proper noun); dismiss buttons (`fa-xmark` 14 uses; the two bespoke `✕` are chip-removes, a distinct concept, one with a "one glyph per concept" comment); and V-2, the MUST-NOT — `node.text`, `nodeById`, `nodeMap`, `parentMap`, `artifact` are all intact.
- **Tests 1611 → 1612.**

### UXP-253b / UXP-247 ✓ Dialogs stopped owning their own shell 🟡 [dialogs] [structural] (RESOLVED 2026-07-26)
- **The filed target was wrong, and was changed with the owner's agreement.** The entry read: *"`openInsertDialog`'s field vocabulary is text / textarea / checkbox; four dialogs need more than that, hand-roll their own DOM, and so forfeit every later fix. Target: grow the field vocabulary and migrate the four."* Research before implementing said that aims at the wrong layer:
  - the fix that was actually forfeited (**UXP-246 drafts**) lived in `openInsertDialog`'s **`cancel()`** — shell, not fields;
  - **22 dialogs**, not 4, each assigned `ioCancel` / `ioReturnFocus` / `aria-label` / `ioBack` / `io-foot` themselves;
  - UXP-246's fix had to be hand-wired into each of the four **separately** (`wireDialogDraft` appeared 4×).
  Forcing 575 lines of genuinely different UI — a mode that rewrites the form, a repeating row list, three date pickers, swatch grids that commit on click — into one submit-oriented field builder would have rewritten four working dialogs for no gain against the actual defect.
- **Shipped instead:** `openDialogShell`, owning teardown + head, draft wiring (through UXP-246's unchanged `wireDialogDraft`), the footer, `cancel`, `ioReturnFocus`, `aria-label`, `ioBack`, and the fresh-open signal. Callers keep **full control of their own body**, so a picker grid or a repeating row list is a first-class citizen instead of a reason to hand-roll everything. `openInsertDialog` + the four named dialogs now build on it.
- **Two capabilities came out of the migration rather than the design.** Draft wiring had to become **deferred** (`shell.wireDraft`), because restoring a draft writes into fields and a caller builds its fields after the head; `openInsertDialog` stays the exception that can pass `read` up front, since its fields restore themselves as they are built. And `openAppearanceDialog`'s private `_apprFresh` became the shell's `fresh` + a `freshOnly` option.
- **Two side effects worth naming.** The §7.6 **footer order is now enforced by construction** (dismiss/neutral first, commit last, danger final) — a rule this program had previously flagged as *"debatable, unenforceable"* is now structurally true. And `cancel` is **literally one function** shared by the dismiss button, the backdrop and global Escape; three copies is precisely how they drifted before UXP-246.
- **The close is a RATCHET, not a rewrite.** 16 dialogs still hand-roll their shell, and migrating all of them at once would be a riskier change than the defect. A test asserts the count of hand-rolled shells **may only ever decrease** (22 before; 17 assignments now, one of which is the shell's own) and fails CI on a 23rd. That is what actually closes the entry: the defect was never "22 dialogs own a shell", it was **"a NEW one can, and will then miss every fix made at the shell"**.
- **No behaviour change, proved before any pin was touched.** Two existing pins asserted the *source shape* of code that moved. Both times the drivers were re-run first: **10/10** hand-rolled surfaces still keep drafts, **8/8** safety properties hold (an abandoned draft never shadows a different pill or point), **9/9** on the shell driver including the nested-in-builder `_dialogRoot` path, and **3/3** on the appearance in-place rebuild (a fresh open restores; a swatch-commit rebuild does not). The rewritten pins assert the same properties at their new home and say why they moved.
- **Tests 1612 → 1614.** Net effect on `index.html` is roughly break-even in lines while removing five copies of the shell.

### UXP-249 ✓ The tap floor got a guard that runs on every PR 🟢 [touch] [structural] (RESOLVED 2026-07-26)
- **The open question, answered.** This was filed with the decision deferred: the obvious fix, "promote the hit-test driver into the repo", collides with CI running plain `node --test` (no npm install, no browser) and with `CLAUDE.md`'s *"verification artifacts stay out of the repo … Playwright installs"*. Owner call this turn: ship a **static ratchet** that binds in the existing CI, and keep the hit-test driver as a documented periodic audit.
- **The proxy's accuracy was MEASURED before it was trusted.** `cursor:pointer` as the stand-in for "tappable" catches **8 of the 9** classes UXP-248 found by driving; the miss is `.fm-title` (`cursor:text`, a click-to-rename field). That number is in the code comment and in `accessibility.md`, so nobody mistakes a green ratchet for a measured floor.
- **Shipped:** `tapFloorCandidates(cssText)` — a pure core (`load-cores.mjs` `need`, seeded pins) returning `{tappable, covered, uncovered}`. Covered = an explicit ≥24px `height`/`min-height`, or **any** rule inside an `@media(hover:none)` block (walked by brace-matching, so a nested rule cannot end the slice early — pinned). The ratchet asserts the uncovered count may only ever **decrease**, and names the offender when it rises.
- **The guard was proved to bite, and the first attempt to prove it was wrong.** Adding a fake `cursor:pointer` class with no treatment must fail CI. The first attempt inserted it **inside** the `hover:none` block, so it counted as covered and the suite stayed green — the test of the test was broken, not the test. Re-inserted into ordinary CSS it failed correctly: `(20 > 19): … fake-new-btn`.
- **It immediately found what the driven audit structurally could not.** 108 classes declare `cursor:pointer`; the proxy surfaced candidates living in `renderBraceMenu`, `renderLinkMenu`, `renderBlPanel`, `updateVarPanelContent` and friends — **caret-anchored menus and panels that no `open*()` entry point reaches**, the same blind spot that hid `#slash-menu` from the P3-1/P3-3 sweep. The static check and the hit-test miss *different* things, which is why both are kept.
- **One real defect fixed, and one candidate withdrawn on measurement.** `.bm-help` (the `{`-menu help glyph) measured **16x17** effective across 5 instances and now measures **24x25**, via the overlay recipe so its 16px circular look is unchanged. `.bl-item` was *also* going to be fixed — until the driver setup was found to have **thrown partway through `renderBlPanel`**, meaning every `.bl-item` reading came from a half-built panel. The padding change was **reverted**: a fix justified by a measurement known to be void is not a fix. `.bl-item` stays on the ratchet as unmeasured, which is the honest state.
- **Measured and left alone:** `.bm-item` (61x49), `.lp-item` (61x33), `.sh-chip` (61x24) clear the floor on their own padding and are recorded in the test's `MEASURED_OK` map with their sizes, so the ratchet's number means something rather than counting known-good controls.
- **Tests 1614 → 1616.** UXP-248's audit re-run after the change: unchanged, the single remaining hit being the `.guide-nav-btn` scroll-pane boundary artifact already recorded there (31x31 when scrolled into view).

### UXP-256 ✓ The toolbar let the level control land on the search box 🔴 [layout] (RESOLVED 2026-07-26)
- **Reported as a regression from recent work. It was not, and that was established before anything was changed.** Rendering the pre-session baseline (`af0ecbf`) against HEAD gave **byte-identical toolbar geometry at every width from 560 to 1280**. The defect accumulated one toolbar button at a time, across four PRs, none of which looks guilty in review:

  | revision | date | toolbar buttons | overlap @1000px |
  |---|---|---|---|
  | `cf74495` | Jul 18 | 9 | 21px |
  | `1978b52` (after `f66c7b0`, "a first-class Rolls-log toolbar toggle") | Jul 20 | 10 | 64px |
  | `c91028b` | Jul 25 | 11 | **105px** |
  | HEAD before the fix | Jul 26 | 11 | **105px, unchanged by this session** |

- **Root cause:** `#search-wrap` is `position:absolute; left:50%` above 950px — viewport-centred and **out of flow** — so the flex row lays `#level-ctl` and the icon strip out as if the search were not there. Nothing could yield: `#level-ctl` is `flex-shrink:0` and the search was not participating at all. Below 951px an existing reflow already makes the search an in-flow flex item, which is why narrow widths were fine; the defect lived in the **gap between that reflow and the widths with enough room**, measured as 951–1215px.
- **Fixed** by extending the proven in-flow treatment across the gap (`@media(min-width:951px) and (max-width:1279px)`). As a real flex item the search cannot be overlapped, because flex distributes rather than stacks. It keeps the wide layout's 220px width so crossing 1280px changes only where the field sits, never its size — a first attempt let it grow to 324px in the band, which snapped back at the breakpoint and read as a glitch.
- **The second reported symptom was NOT a defect.** The wordmark hiding and the search collapsing to a magnifier is the intended ≤560px design, and its breakpoint is **identical across every revision back to Jul 18** (580px: wordmark shown, 190px field · 560px: hidden, 40px icon). Reported as checked rather than quietly fixed.
- **Guard:** a ratchet on the toolbar button count (11). The 1279px bound carries ~65px of measured headroom at today's count; a 12th button eats it, so adding one now fails CI and forces a re-measure instead of silently re-creating the defect at a slightly different width.
- **Verified by driving:** overlap measured as rectangle intersection between `#search-wrap` and `#level-ctl` across 14 widths from 520 to 1400 — **4 widths overlapping before, 0 after** — plus before/after screenshots at 1000px showing "1 2 `Ctrl+F` ll" crammed inside the search field, then cleanly separated.

### UXP-252 ✓ "Roll palette" became "Roll menu" 🟢 [vocab] (RESOLVED 2026-07-26)
- §1 maps a keyboard-navigable overlay list to **menu** and bans **palette**. Filed as needing an owner call because it renames a shipped feature; measurement made it cheap: **two user-facing strings** (a GUIDE entry title and one command desc), and the picker never displays its own name in the UI.
- Renamed those two plus the two code comments and the `backlog.md` mention, so a grep for the old name finds nothing. **The GUIDE entry id stays `roll-palette`** — an internal key referenced by the concept-guide drift guard, where renaming would break `covers:[…]` for no user benefit.

### UXP-254 ✓ A dice-shaped typo now says why 🟢 [feedback] (RESOLVED 2026-07-26)
- The last silent rejected input. **Measured before deciding:** `2d`, `2d0`, `0d6`, `2d6x` and `2d6kX` were ALREADY cued; the entire gap was `\d+d` followed by a letter (`2dX`, `2dz`, `4dQ`, `1dK`, `20dY`). So the doctrine change the entry worried about shrank to one tight pattern.
- **Two things caught it before it shipped wrong.** First, a looser `\d+d[a-z]` broke an existing pin recording that **`{2days}` is prose, not a dice attempt** — a decision already made, so the tail was tightened to a single letter rather than the pin relaxed. Second, and worse: placed with the keyword commits, the rule fired **before `parseDice` ever ran** and condemned **valid Fate dice** (`{4dF}`, documented in `guide/generating-text.md`). Dice is the only form whose keyword is its SHAPE, so the commit has to sit *after* the parse. Verified against the pre-change build that `{4dF}` was artifact before and is artifact again, and that `{2d6kh3}` was already invalid (keeping 3 of 2 dice is impossible) rather than newly broken.
- It routes into the richer `.brace-attempt` cue with a real reason — *"Dice need a number of sides: try {2d6}. "X" is not a number."* — driven in both render and edit mode.

### UXP-257 ✓ The wrapped toolbar row was indented instead of starting at the left 🟡 [layout] (RESOLVED 2026-07-26)
- **Found by the owner immediately after UXP-256 shipped, at a width UXP-256 did not touch.** Confirmed pre-existing the same way: baseline vs deployed is **identical at every width from 560 down to 340**, and UXP-256's media query is bounded to 951–1279px so it cannot reach here.
- **Root cause:** at ≤560px the row is `flex-wrap:wrap` and `#tbtn-cluster` keeps `margin-left:auto`. On a line SHARED with the logo and search that means "sit at the right edge", which is right. Once the cluster wraps to a line of its own there is nothing left to push against, so the auto margin only indents it — **145px of dead space at 510px**, shrinking to the row padding by 370px.
- **Fixed** by dropping the auto margin below the measured wrap point (510px at today's 11 buttons), so the icons start at the left edge of their own row. Above it the single-row right-alignment is untouched.
- **The first attempt broke more than it fixed, and the measurement caught it.** Adding the rule *inside* the `@media(max-width:560px)` block meant inserting a `}` mid-block, which detached every rule after it — including `#dirty-dot + .spacer{display:none}` — so 520–560px started wrapping too. The re-measure showed those widths flipping from unwrapped to wrapped, which is the only reason it was noticed; the rule now sits after the block, brace-walked to find its true end.
- **Verified:** 520–560 unwrapped and right-aligned exactly as before (gaps 155–195px, unchanged); ≤510 wrapped with an 8px left gap, which is the row padding. UXP-256's overlap check re-run across all 14 widths: still 0.
