...
| Dates (start + due) | `start: YYYY-MM-DD` and/or `due: YYYY-MM-DD` (also `today`, `today+N`, `tomorrow`) as **property** values | per-point scheduling as a start→due **range** (project-management style). Recorded UXP-20 decision (2026-06-13, extended 2026-06-13): dates live as `start`/`due` properties in `node.props` — zero new authoring syntax (the properties system already exists); the `due:`/`start:` search operators are date-aware extensions of the existing `key:value` search row. Displayed as date-smart chips (Today / Tomorrow / Mon / 3d overdue; the start chip leads with `▸`). Front doors: `/due` slash verb (labelled **"Schedule"**), bullet menu "Set / Edit dates", Agenda strip (toolbar button). The Agenda is a **two-row horizontal strip** in the toolbar (Due row + Running row) with a left toggle group (Running / Done) — a horizontal strip, not a vertical sidebar, so it never constrains the outline width or obstructs a mobile screen; calendar / timeline view buttons are planned for the same control group. |
