...
| **Grammar engine** | `{…}` → `{= expr}` (incl. subtree rollups `{= sum\|avg\|count\|min\|max(prop)}`), `{NdM}`, `{a\|b 2\|c}` (a weight is a number **or** a `{= expr}` — A5 dynamic odds), `{cond: then \| else}`, `{shuffle\|cycle\|once\|stopping: a \| b \| c}`, **`{Nx: template}`** (repeat — emit template N times, 1–99), `{rule}`/`{table}`/`{chain}`/`{var}`, **`{ref.mod}`** (A1 text modifier — `cap`/`title`/`upper`/`lower`/`a`/`s`) | **ALL** generative / computed inline content |
