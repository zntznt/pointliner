...
| **Grammar engine** | `{…}` → `{= expr}` (incl. subtree rollups `{= sum\|avg\|count(prop)}`), `{NdM}`, `{a\|b 2\|c}`, `{cond: then \| else}`, `{shuffle\|cycle\|once\|stopping: a \| b \| c}`, `{rule}`/`{table}`/`{chain}`, `{var}` | **ALL** generative / computed inline content |
