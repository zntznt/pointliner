...
| Variables | ✅ | ✅ (`{` picker + variables panel; UXP-9) | ⚠️ (pill: complete — tabindex/Enter/Space + per-state aria-labels for ↻/?/— errors; vars panel: role=region + close-btn named, but no aria-live on content-refresh — UXP-38) | ✅ (live ↻/？/— error codes + distinct aria-labels per state; no stale fallback — UXP-34) |
...
| Footnotes / hashtags / emoji | ✅ | ✅ (`#` tag picker — UXP-10) | ⚠️ (fn-panel: named + Enter/Space-operable — complete; hashtag `<a>` elements have no tabindex/keydown — element-level P3-1 gap; capability keyboard door exists via search + `#` picker; same disposition as todo-picker chips — UXP-39) | ✅ |
