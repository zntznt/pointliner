...
| Table `↑/↓` | Cell above / below; at the top/bottom edge, **exit** the base to the adjacent outline point | Plain-arrows rule: the caret moves within the cell's text first; navigation fires at the first/last line — same convention as `↑/↓` between points (which likewise **enter** a base's first/last cell when arrowing past it). A navigation-placed (type-to-overwrite) selection acts as a spreadsheet cursor: `↑/↓` move cells directly. That nav-placed selection never opens the selection toolbar; an intentional one (mouse drag, `Shift+arrows`, `Ctrl/⌘+A`) does. `@` in a cell opens the insert menu (sans Table — no table-in-base — and Footnote, which is point-level) |
