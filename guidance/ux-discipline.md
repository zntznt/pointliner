...
| Family | Syntax | Owns |
|---|---|---|
| Markdown | `#`–`######`, `>`, ```` ``` ````/`~~~`, `---`/`***`, `-`/`1.`/`- [ ]`, `**`/`__`, `*`/`_`, `++`, `` ` ``, `~~`, `==`, `[text](url)`, `![alt](url)`, bare-URL autolink, GFM pipe table (header + `---` delimiter row), definition list (`term` then `: def`) | block + inline text formatting, links, images, autolinks, static pipe tables, definition lists |
| Emoji | `:shortcode:` | emoji |
| Hashtag | `#tag`, **nested** `#tag/sub`, hyphens ok (`#plain-tag`) | tags. Nesting is an extension of the existing `#` form (segments `[\w-]+` joined by `/`), **no new sigil or delimiter**; tag search is hierarchical (`#thread` matches `#thread/torn-letter`). Grammar mirrored in `mdInline` / `collectTags` / search-parser |
...
| **Grammar engine** | `{…}` → `{= expr}` (incl. subtree rollups `{= sum\|avg\|count\|min\|max(prop)}`), `{NdM}`, `{a\|b 2\|c}` (a weight is a number **or** a `{= expr}` — A5 dynamic odds), `{cond: then \| else}`, `{shuffle\|cycle\|once\|stopping: a \| b \| c}`, **`{markov: a→b, b→c}`** (typed markov chain — `markov:` keyword then comma-separated `from→to weight` transitions, an anonymous twin of the `@`/Markov dialog), **`{seq Name: active \| done}`** (typed sequence declaration — `seq` keyword + a name + the `parseSequence` states tail; a NAMED, atomic `[[seq:key]]` pill, the typed twin of the `@`/Sequence dialog), **`{Nx: template}`** (repeat — emit template N times, 1–99), `{rule}`/`{table}`/`{chain}`/`{var}`, **`{name := expr}`** (typed variable declaration, formula or random pick — the typed twin of the `@`/Variable dialog; `:=` sniffed before the `:`-splitting paths so it never collides with rule lines or `{= }`. Stage A of guidance/typed-var-declaration-proposal.md), **`{"literal"}`** (a quoted string literal — the text-value escape so a bare word stays a rule lookup; nested `{…}` still interpolate inside), **`{ref.mod}`** (A1 text modifier — `cap`/`title`/`upper`/`lower`/`a`/`s`/`ed`/`ord`) | **ALL** generative / computed inline content |
...
| `Ctrl/⌘ + Shift + P` | Cycle to-do priority forward (edit mode) | Advances `#A → #B → #C → (none) → #A …`. No-op when no state keyword is present. Announces via `#a11y-live`. |
