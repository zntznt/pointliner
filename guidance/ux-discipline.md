...
| `/verb:value` (opted-in verbs only) | Apply a command **with an inline argument**, skipping its dialog | Today: `/due:tomorrow`, `/start:2026-07-01` (also `today`, `today±N`). **Only arg-verbs consume the `:value`**; on any other verb the colon is plain text (`/quote:x` → quote, `:x` stays). Reuses the `/` door — **not** a new sigil, does not grow the §2 inventory (P5). See §7.1a for the build contract (narrow gate, trigger-pinned strip, P4 on a bad value) — **UXP-69** |
