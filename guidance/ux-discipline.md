...
| **Grammar engine** | `{…}` → `{= expr}` (incl. subtree rollups `{= sum\|avg\|count(prop)}`), `{NdM}`, `{a\|b 2\|c}` (a weight is a number **or** a `{= expr}` — A5 dynamic odds), `{cond: then \| else}`, `{shuffle\|cycle\|once\|stopping: a \| b \| c}`, `{rule}`/`{table}`/`{chain}`/`{var}`, **`{ref.mod}`** (A1 text modifier — `cap`/`title`/`upper`/`lower`/`a`/`s`) | **ALL** generative / computed inline content |
...
| Footnotes / hashtags / emoji | ✅ | ✅ (`#` tag picker — UXP-10) | ✅ (fn-panel named + Enter/Space-operable; hashtag chips `role=button` + `tabindex="-1"` + `aria-label` + Enter/Space filter twin beside `mousedown` — caret invariant, mirrors note-ind/prop-chip — UXP-39; emoji is non-interactive text) | ✅ |
| Search | ✅ | ✅ | ✅ (aria-label + aria-describedby on search box; saved-search chips role=button + Enter/Space/Delete/Esc; ↓ jumps to outline) | ✅ |
| Dates / agenda (List · Timeline · Calendar) | ✅ (click/Enter zooms everywhere; Calendar arrow-key grid nav follows the date-picker convention) | ✅ (toolbar Agenda button + `/due` Schedule verb + bullet menu; `?` panel **Dates & agenda** section + search legend document `due:`/`start:` — UXP-37) | ✅ (toggles/chips/bars/cells: `role`+`tabindex`+`aria-label` with `keydown` beside `mousedown` — caret invariant; Calendar cells `role=gridcell` + roving tabindex) | ✅ (no silent failure; empty-state hints route to `/due`) |
