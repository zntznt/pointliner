...
| Due date | `due: YYYY-MM-DD` (also `today`, `today+N`) as a **property** value | per-point scheduling. Recorded UXP-20 decision (2026-06-13): dates live as a `due` property in `node.props` — zero new authoring syntax (the properties system already exists); the `due:` search operator is a date-aware extension of the existing `key:value` search row. Displayed as a date-smart chip (Today / Tomorrow / Mon / 3d overdue). Front doors: `/due` slash verb, bullet menu "Set due date", Agenda panel (toolbar button). |
