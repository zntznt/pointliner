...
- **Variance in one click.** When a heading's rows carry two number columns (a quote and an actual, an
  estimate and an invoice), a + Variance door writes the over-or-under for you: the difference on each
  row and the total on the heading, so a budget-vs-actual reads itself with no formula typed.
...
- **Action buttons.** Where a meter shows a number, `{hp -= 1d6}` **changes** one: a pill you click to
  spend, take damage or refill, so a live session never retypes a stat. `+=`, `-=`, `*=`, `/=` and `=`,
  with a live expression on the right (`{gold += 2d6*10}`). One click, one undo.
  ([action buttons](tasks-and-organizing.md#action-buttons))
